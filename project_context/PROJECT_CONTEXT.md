...
### 1.2.1 2026-07-17 最新提交与工作区进展

- 当前 Git HEAD 为 `6b67b55`（`refactor(web): consolidate UI on control console`），`master`、`origin/master`、`origin/HEAD` 一致。自 2026-07-14 文档基线后，已提交模式化设备预检（`ef4fb83`）、控制台实时状态同步（`dbc02d8`）、运行启动与机械臂状态修正（`0e0b8df`），以及 Web UI 统一到数字孪生控制台并删除旧正式页面、旧 FastAPI 原型和重复静态资源（`6b67b55`）。
- 当前仍有大规模未提交修订：工作流配置模板与步骤时间线、`WorkflowVersion.PlanningRulesJson` 持久化及迁移、DAB 配比/预检/运行衔接、试剂扫描 Mock 闭环、通道与玻片位状态、运行账本与数字孪生快照、Mock 运行态重置、严格模式预检，以及相应后端/浏览器测试正在收口。
- 新增迁移 `20260715173748_PersistWorkflowPlanningRules`，为 `workflow_versions` 增加非空 `planning_rules_json`（默认 `{}`）；当前迁移文件尚未提交，正式数据库迁移一致性结论需待测试修复和提交后重新确认。
- 新增 `MockRuntimeResetService` 与 `/api/mock-runtime/reset`，并禁用旧 `/api/system/reset` 的 `MockRuntimeStore` 重置路径；该能力只允许用于 Twin/Mock 演示数据和运行态复位，不得作为 Real 设备复位实现。
- 2026-07-17 执行 `dotnet test backend/Stainer.Tests/Stainer.Tests.csproj --no-restore` **未通过编译**：测试仍引用已移除的 `ReferenceDataSeeder.ManualHeWorkflowCode` / `ManualIhcWorkflowCode`，共报告 10 个 CS0117。当前完整后端回归状态为“阻塞于测试编译错误”，不得写成已通过。
- 工作区包含两份 `data/stainer.db.bak.clear-slides.*` 数据库备份、大量 `tmp-build/*` 验收构建目录、`tmp-dbprobe` 和两个新增浏览器验收脚本；提交前必须确认哪些属于需保留证据、哪些属于临时产物，避免再次把大体量构建输出误纳入正式源码基线。
- 真实 COM/CAN/TCP、真实扫码、真实温控/混匀/液路动作仍未执行；上述提交和工作区能力均不改变真实硬件阶段的 fail-closed 边界。
...
| Workflow 通道关联模型评估 | 已完成（评估） | 现有 WorkflowDefinition / WorkflowVersion / ChannelBatch 已满足“同通道 4 玻片共享单一 Workflow、不同通道可选不同 Workflow”，无需另建关联模型；2026-07-17 工作区正为规划规则扩展 PlanningRulesJson 字段和迁移，尚未收口 |
...
- 当前阶段结论：坐标 Z 语义、LiquidClass 参数补齐与工程手动移液测试 API 均已完成并提交；扫码设备职责与坐标语义已按当前确认口径文档化。Workflow 配置、规划规则、DAB/预检与运行衔接正在 2026-07-17 工作区继续开发，尚未完成完整回归。
- 历史主项目测试基线：曾有 **218/218 tests passed，TEST_EXIT=0**（含 `EngineeringPipettingApiTests`）；该结果不代表当前工作区。2026-07-17 当前完整测试因 10 个 CS0117 编译错误未通过。
...
- 正式数据库：历史基线曾验证迁移一致、无 pending migration；当前工作区新增 `PersistWorkflowPlanningRules` 迁移，尚待应用和重新验证，当前不得沿用旧结论。
...
- 2026-07-17 修订补充了此前未覆盖的规划规则持久化需求：在既有 `WorkflowVersion` 上增加 `PlanningRulesJson` 并新增 Migration；因此本节原有“无需新增任何字段或 Migration”仅是历史判断，已被当前需求扩展修正。
...
- 当前正在开发配置模板、时间线编辑和规划规则持久化，须先完成迁移、后端回归和浏览器验收；后续执行/调度仍是独立能力，不能因配置页完成而视为执行器完成。
...
Workflow 通道关联模型评估完成（无需另建关联模型；ChannelBatch→单一 WorkflowVersion→4 玻片共享已强制；规划规则字段扩展仍在 2026-07-17 工作区收口）
```
