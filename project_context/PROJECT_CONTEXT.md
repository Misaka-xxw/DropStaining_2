...
### 1.2.1 2026-07-14 最新提交进展

- 当前 Git HEAD 为 `cd52b22`（`feat:完成设备控制与工程配置接入`），远端 `origin/master` 已指向同一提交，工作区干净。前一提交 `a67965f` 已完成数字孪生控制台运行生命周期与扫码流程接线；本次提交在此基础上继续完成工程/配置控件接入。
- 数字孪生工程/配置控件的温控板启停与查询、混匀启停与查询、液体类型参数读取/保存/新建、扫码器创建与 Restart / Calibration Light / ROI 控制、液位阈值保存，以及样本清洗启停均已接入现有正式后端 API；这些属于 Mock/工程控制台软件能力，不代表真实硬件已验证。
...
- 本轮源代码、测试、浏览器验收脚本调整和验收构建产物已提交。提交共涉及 449 个文件，其中包含 `tmp-build/acceptance-build`、`tmp-build/final-review`、`tmp-build/review-build`、`tmp-build/acceptance-script.js` 及 `tmp-build/webcheck` 更新；后续应评估这些临时产物是否需要长期纳入版本控制，并完善忽略/留存规则。
- 真实 COM/CAN/TCP、真实扫码、真实温控/混匀/液路动作均未执行；正式发布结论仍需完整后端回归和浏览器级验收记录支持。
...
- 当前阶段结论：坐标 Z 语义、LiquidClass 参数补齐与工程手动移液测试 API 均已完成并提交；扫码设备职责与坐标语义已按当前确认口径文档化，后续需落地 DCR55 工具偏移、Slot / `ScannerRegion` 样本扫码位和主控 `0x08` 试剂扫码映射；Workflow 配置模型经评估无需继续开发，后续 Workflow 相关重点为执行/调度能力评估（独立任务，不代表立即进入开发）。
