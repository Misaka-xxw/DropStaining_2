...
- **当前阶段定位**：
  - Mock 业务能力、正式数据模型和正式 Web HMI 接入已形成提交基线；2026-07-21 当前工作树完整后端回归 313/313 通过，`/control-console` 单页浏览器验收已重跑通过；
...
- 当前 Git HEAD 为 `505f73b`（`test: align regression cases with current contracts`），`master` 与 `origin/master` 指向同一提交；本轮基线审核前跟踪文件干净，另有未跟踪的 `tmp-bridge-build/`、`tmp-hardware-tests-build/`。二者已被 `.gitignore` 忽略，本轮未删除目录；忽略临时构建目录不代表其中内容已经提交或成为正式验收证据。
...
- 2026-07-21 已修订新增 `Ab` 液类后仍假定“只有一个启用液类”的测试选择器，统一明确选择 `FactoryGeneral-v1`；同时按已确认业务规则将 Published Workflow 测试改为允许原地修改、删除仍禁止。实际执行 `dotnet test backend/Stainer.Tests/Stainer.Tests.csproj --no-restore`：**总计 313，313 通过、0 失败、0 跳过**。Published Workflow 已确认允许原地修改、删除仍被禁止；ChannelBatch 选择 Workflow 时保存 `WorkflowSnapshotJson`，已创建批次具备冻结快照，`WorkflowAssignmentHistory` 和 `AuditLog` 提供变更追溯，相关专项测试 16/16 通过。尚未完全确认所有历史页面、历史 API、导出和 ReadModel 是否始终使用冻结快照，也尚未排除通过 `WorkflowVersionId` 重新读取当前可变 Published 版本的全部路径，因此不得写成历史漂移风险已完全排除。本轮测试修订尚未提交。
- BASELINE-CLOSE-01 首次构建曾报告一次 CS0162；后续普通 Release 构建和 `NoIncremental=true` 构建均为 **0 warning / 0 error**，当前无法稳定复现。`AllowPublishedWorkflowChildEdits = true` 之后存在结构性不可达分支；本轮未修改生产代码，该项当前不阻塞基线提交判断，但可作为后续代码清理项。
...
- 浏览器验收：历史基线曾完成；2026-07-21 当前 `/control-console` 单页回归已复核通过，后续代码变化后仍需重跑。
- 数字孪生 XY 导入及门禁验证：已完成。
...
- 主项目测试：历史基线曾为 **218/218 tests passed**；2026-07-20 一度为 252/313 通过。2026-07-21 修订测试数据选择与 Published Workflow 契约后，当前工作树完整回归为 **313/313 通过、0 失败、0 跳过**；BASELINE-CLOSE-01 复核中再次执行正式 Release 构建和完整后端测试，仍为 **313/313 通过、0 失败、0 跳过**。
...
- 数据库迁移复核：`PersistWorkflowPlanningRules`、`AddSerialConnectionProfile`、`AddPrecisionCalibrationProfile`、`AddMixerParameterProfile`、`AddWashValveConfigProfile` 等迁移已进入提交历史；BASELINE-CLOSE-01 使用隔离空数据库完整应用 37 个 Migration 成功，首个 Migration 为 `20260624021457_InitialInfrastructure`，最后 Migration 为 `20260720050732_AddWashValveConfigProfile`；`integrity_check`、`foreign_key_check`、关键基础查询均通过，且无 pending model changes。正式数据库未被本轮测试修改。正式旧数据升级兼容验证：未完成；当前仅完成空数据库全量迁移验证，不能将空数据库迁移成功写成旧数据升级兼容成功。
...
- 配置模板、时间线编辑和规划规则持久化已进入提交基线；Mock Workflow 执行器、运行状态机、运行账本和资源租约也已实现。当前完整后端回归 313/313 通过，隔离空数据库 37 个 Migration 全量应用、完整性/外键/基础查询和无 pending model changes 已复核通过，`/control-console` 浏览器验收已复核通过；正式旧数据升级兼容验证仍未完成，Real 设备执行编排仍是独立且未完成的能力。
...
> 当前工作树完整后端回归已恢复为 313/313 通过，隔离空数据库 37 个 Migration 全量应用、完整性/外键/基础查询和无 pending model changes 已复核通过，`/control-console` 单页浏览器验收已复核通过；正式旧数据升级兼容验证和真实设备现场验收仍未完成，不能据此视为发布可用。
