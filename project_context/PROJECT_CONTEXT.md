...
- 扫码器配置持久化基础：`ScannerProfile`、`ScannerRegion`、`scanner_profiles`、`scanner_regions` 与配置管理 API 已完成；当前仅表示扫码器配置持久化与管理能力，不代表真实扫码器控制能力。
...
当前已完成的是“扫码器配置持久化与管理能力”，不是“真实扫码器控制能力”。

已完成：

- 新增 `ScannerProfile` 配置模型；
- 新增 `ScannerRegion` 扫码区域模型；
- 新增数据库表：
  - `scanner_profiles`；
  - `scanner_regions`；
- 已支持后端配置管理 API：
...
扫码器配置持久化与管理 API（ScannerProfile/ScannerRegion；不代表真实控制）
```
