...
- 新增/重构：告警中心、历史与导出、启动校验弹窗、DAB 面板、A-D 抽屉/16 Slot 总览。
- 重构样本确认：支持通灵 IHC 码、医院码/LIS 分支、HE 手动确认的 UI 流程。
- 重构试剂管理：按 ch1-ch5、R1-R40 展示 EMPTY/VALID/INVALID 和试剂解析详情。
- 重构运行页：移除运行中追加玻片入口，普通停止改为整机停止，启动前执行不可绕过校验。
- 重构配置/工程师模式：覆盖流程版本、映射、试剂目录、Liquid Class、坐标、通讯、扫码、温控、泵/液位和配置版本。
- 保持登录角色为管理员 admin 与实验员 operator；工程师模式作为管理员受控页面，而非独立登录角色。
