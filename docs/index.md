...
- `appsettings.json` / `appsettings.Development.json` — 运行配置：`Database`、`Device`（Mode/主控串口/DCR55/试剂硬件开关/通道硬件状态映射/SOCON 试剂动作 IPC）、日志、安全。
...
- `RealDeviceReadBoundary.cs` — 真机读侧契约：`IDeviceByteTransport`（字节交换/接收）+ 窄接口 `IChannelHardwareStatusDeviceReader`（主控工作/节点状态）+ 完整 `IRealDeviceReadAdapter`（结构化只读）+ 状态/端点常量。
- `ReagentHardwareActionBoundary.cs` — 试剂真实动作端口：坐标/安全 Z/动作 Z/体积等工程量请求及结构化结果，不暴露 COM、NodeID 或 SDK 类型。

#### Application/Services

业务服务层（约 90 个文件）。按职责分组：
...
- **液路/运动/温控/供水**：`FluidicsControlService.cs`（泵/混匀/液位/清洗/洗针）、`MotionControlService.cs`（机械臂/针/移液）、`RobotMotionPrimitives.cs`（机械臂运动原语端口 `IRobotMotionPrimitives` + Mock/Unavailable/SOCON 三实现）、`ThermalControlService.cs`（温控点/制冷）、`WaterSupplyControlService.cs`（每通道供水，Real 下 fail-closed）。
- **DAB**：`DabLifecycleService.cs`（DAB 批次全生命周期）、`DabExpiryHostedService.cs`（过期扫描后台服务）。
- **工程**：`EngineeringSessionService.cs`（工程会话）、`EngineeringWriteService.cs`/`EngineeringConfigService.cs`/`EngineeringQueryService.cs`/`EngineeringDiagnosticService.cs`（工程写/配置/查询/诊断）、`EngineeringPipettingService.cs`（工程移液测试）、`RobotArmProcessActionService.cs`（移动到安全高度与吸液/排液/混匀/清洗原子动作的流程编排）、`RobotArmAtomicActionService.cs`（工艺原子动作编排底层 `IRobotMotionPrimitives`，含 TakeLiquid/PrepareMix/DispenseLiquid/WashInner/WashOuter 五动作）、`RobotArmAtomicActionRecorder.cs`（原子动作净效果写入 Mock 运行状态/流水账）、`RobotArmEngineeringService.cs`（机械臂工程隔离测试，包装原子动作+XY 平移并加幂等/审计外壳）。
- **设备/预检/模式**：`DeviceControlService.cs`（设备状态/故障控制）、`ChannelHardwareStatusService.cs`（独立读取主控工作/节点状态并按配置映射 A–D，不写业务通道状态）、`ReagentPositionHardwareService.cs`（工程会话保护的试剂 XY/Z 现场运动）、`ReagentPositionConfigService.cs`（试剂位对象配置读写）、`CoordinatePointHardwareService.cs`（通用坐标点 XY/Z SOCON 现场移动，覆盖玻片/A-B液/配液/清洗位）、`DeviceInitializationService.cs`（设备初始化步骤）、`DevicePrecheckService.cs`（11 项预检）、`DeviceModeService.cs`（Mock/Real 模式）、`PreHardwareReadinessService.cs`（启动就绪门禁）、`PreflightValidationService.cs`（预检报告聚合）、`ScannerControlService.cs`/`ScannerConfigurationService.cs`（扫码器控制/配置）。
...
- `DeviceReadModels.cs`、`ChannelHardwareStatusReadModels.cs`、`ScannerConfigurationReadModels.cs` — 设备初始化/故障/试剂 QR、独立真实硬件通道状态快照、扫码器档案/区域。
- `EngineeringReadModels.cs`、`PrecheckReadModels.cs`、`TraceabilityReadModels.cs` — 工程布局/坐标/液类、预检项/报告、追溯列表（带分页）。
- `ReagentPositionConfigReadModels.cs`、`CoordinatePointHardwareReadModels.cs`、`RobotArmEngineeringReadModels.cs`、`SerialDebugReadModels.cs` — 试剂位配置、坐标点现场移动、机械臂工程测试、串口调试响应 DTO。
...
- `DabRequests.cs`、`EngineeringRequests.cs` — DAB 批次/制备/消耗、工程会话/配置导入/坐标导入/坐标点与机械臂现场移动。
- `ReagentPositionConfigRequests.cs`、`RobotArmAtomicRequests.cs`、`SerialDebugRequests.cs` — 试剂位配置/移动、机械臂原子动作（含 `RobotArmAtomicHeights`）、串口调试请求。
...
- `SoconReagentHardwareActionClient.cs` — 主后端到本机 x86 SOCON Bridge 的长度前缀命名管道客户端，编排安全 Z→XY→动作/探液→回安全 Z，默认关闭且失败闭合。
- `SoconRobotMotionPrimitives.cs` — 双针机械臂运动原语的 SOCON 实现（MoveXY/MoveZ(Z1,Z2)/Aspirate/Dispense 复用已审核 Z-SOPA；WashOuter fail-closed），DI 三选二（Real+Enabled?Socon:Unavailable，Mock 独立）。
- `SerialDebugService.cs` — 真实 COM 字节收发调试服务（admin-only，绕开 `IDeviceAdapter` 运行链路，用于环回/串口助手验证）。
...
- `WebHostEndpointExtensions.DeviceOperations.cs` — 系统信息/设备模式/设备与温控模块状态控制，含独立只读 `/api/device/channels/hardware-status`。
...
- `WebHostEndpointExtensions.ReagentPositionConfig.cs` — 试剂位对象配置 GET/PUT（admin）+ `{rackCode}/move-test` 现场移动端点（三重门禁）。
- `WebHostEndpointExtensions.CoordinatePoint.cs` — 通用坐标点 `{pointCode}/move-test` 现场移动端点（三重门禁，Real-only）。
- `WebHostEndpointExtensions.RobotArm.cs` — 机械臂工程隔离测试端点 `/api/engineering/robot-arm/move-test`、`/atomic-action`（三重门禁）。
...
独立 .NET Framework 4.5.2 x86 控制台进程，通过 Windows 命名管道提供受门禁保护的只读状态与试剂动作 IPC；动作必须同时满足 `--enable-real-actions`、本地 `realActionsEnabled`、SDK/连接校验、NodeID 白名单、轴标定和软限位，支持受限 MoveX/Y/Z、Z-SOPA 吸排液、液面探测、等待与停止；主项目经 `SoconReagentHardwareActionClient` 接入。

- `Program.cs` — 进程入口：解析 `--self-test`/`--enable-real-read-only`/`--enable-real-actions`，单实例 mutex，构造 processor 并运行 BridgeHost。
- `BridgeHost.cs` — 命名管道服务器循环（ACL 仅当前用户/LocalSystem，长度前缀 UTF-8 JSON 帧）。
- `BridgeRequestProcessor.cs` — 命令分派核心：只读与受限动作命令、双开/部署/白名单/标定/量程门禁、session 状态机及去敏感化。
...
- `ISoconReadOnlyAdapter.cs` — 只读适配器与扩展 `ISoconActionAdapter` 抽象；动作面仅开放 Move/Aspirate/Dispense/DetectLiquid/Stop。
- `ReflectionBasedSoconReadOnlyAdapter.cs` — 反射加载 `SOCON.API.dll` 的真实适配器，绑定只读操作及已审核的 Move/Z-SOPA/Wait/Stop 动作。
...
- `SelfTestRunner.cs` — `--self-test` 离线自检（当前 98 项检查，不连真实 SDK/硬件），覆盖动作开关、白名单与量程拒绝。
