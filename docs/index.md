...
- `appsettings.json` / `appsettings.Development.json` — 运行配置：`Database`、`Device`（Mode/主控串口/DCR55/试剂硬件开关/通道硬件状态映射）、日志、安全。
...
- **设备/预检/模式**：`DeviceControlService.cs`（设备状态/故障控制）、`ChannelHardwareStatusService.cs`（独立读取主控工作/节点状态并按配置映射 A–D，不写业务通道状态）、`DeviceInitializationService.cs`（设备初始化步骤）、`DevicePrecheckService.cs`（11 项预检）、`DeviceModeService.cs`（Mock/Real 模式）、`PreHardwareReadinessService.cs`（启动就绪门禁）、`PreflightValidationService.cs`（预检报告聚合）、`ScannerControlService.cs`/`ScannerConfigurationService.cs`（扫码器控制/配置）。
