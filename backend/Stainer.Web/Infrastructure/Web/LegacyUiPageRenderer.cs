...
"""
    ...
    {
        return """<section class="modern-card"><div class="section-title"><div><h2>告警查询</h2><p>活动中 / 已确认 / 已处理告警均来自正式数据库。</p></div><div class="button-row"><button class="btn btn-soft" onclick="loadTraceAlarms()">查询</button><button class="btn btn-soft" onclick="exportTraceCsv('alarms')">导出告警 CSV</button></div></div><div class="inline-form touch-form"><label>状态<select class="input" id="alarmStatusFilter"><option value="">全部</option><option value="Active">活动中</option><option value="Acknowledged">已确认</option><option value="Resolved">已处理</option></select></label><label>级别<select class="input" id="alarmSeverityFilter"><option value="">全部</option><option value="Info">信息</option><option value="Warning">警告</option><option value="Error">错误</option><option value="Critical">严重</option></select></label><label>通道<select class="input" id="alarmChannelFilter"><option value="">全部</option><option>A</option><option>B</option><option>C</option><option>D</option></select></label></div><div class="alarm-list" id="alarmList"></div></section><section class="modern-card"><div class="section-title"><h2>告警处理记录</h2><button class="btn btn-soft" onclick="location.href='/history'">历史与导出</button></div><div class="timeline large" id="alertLogs"></div></section>""";
    }

    private static string HistoryContent()
    {
        return """<section class="modern-card"><div class="section-title"><div><h2>筛选条件</h2><p>历史运行、玻片、试剂消耗、DAB、告警和审计均来自正式数据库。</p></div><div class="button-row"><button class="btn btn-primary" onclick="loadTraceHistory()">查询</button><button class="btn btn-soft" onclick="exportTraceCsv('history-runs')">导出运行 CSV</button><button class="btn btn-soft" onclick="exportTraceCsv('reagents')">导出试剂消耗 CSV</button></div></div><div class="inline-form touch-form"><label>开始时间<input class="input" id="historyFromFilter" type="datetime-local"></label><label>结束时间<input class="input" id="historyToFilter" type="datetime-local"></label><label>运行状态<input class="input" id="historyStatusFilter" placeholder="例如：已完成"></label><label>通道<select class="input" id="historyChannelFilter"><option value="">全部</option><option>A</option><option>B</option><option>C</option><option>D</option></select></label><label>Slot<input class="input" id="historySlotFilter" placeholder="A-01"></label><label>HE/IHC<select class="input" id="historyExperimentFilter"><option value="">全部</option><option>HE</option><option>IHC</option></select></label><label>流程<input class="input" id="historyWorkflowFilter" placeholder="流程名称/版本"></label><label>样本码<input class="input" id="historySampleFilter" placeholder="样本码"></label><label>一抗<input class="input" id="historyPrimaryFilter" placeholder="001"></label><label>试剂<input class="input" id="historyReagentFilter" placeholder="HEM"></label><label>批号<input class="input" id="historyBatchFilter" placeholder="20260101"></label><label>操作人<input class="input" id="historyOperatorFilter" placeholder="operator"></label></div></section><section class="split-grid"><article class="modern-card"><div class="section-title"><h2>运行 / 通道 / 玻片记录</h2></div><div class="data-table history-table" id="historySlides"></div></article><article class="modern-card"><div class="section-title"><h2>试剂与消耗追溯</h2></div><div class="data-table history-table" id="historyReagents"></div></article></section><section class="modern-card"><div class="section-title"><h2>用户操作与配置版本日志</h2><div class="button-row"><button class="btn btn-soft" onclick="loadTraceAudit()">刷新审计</button><button class="btn btn-soft" onclick="exportTraceCsv('audit')">导出审计 CSV</button></div></div><div class="timeline large" id="historyLogs"></div></section>""";
    }
    ...
    private static string EngineerContent()
    {
        ...
        <section class="split-grid"><article class="modern-card"><div class="section-title"><div><h2>坐标版本</h2><p>草稿 → 已发布 → 已启用 / 未启用</p></div><button class="btn btn-primary" onclick="createCoordinateVersion()">复制为 Draft</button></div><div class="data-table" id="coordinateVersionTable"></div></article><article class="modern-card"><div class="section-title"><div><h2>Liquid Class 版本</h2><p>草稿 → 已发布 → 已启用 / 已停用</p></div><button class="btn btn-primary" onclick="createLiquidClassVersion()">复制为 Draft</button></div><div class="data-table" id="liquidClassVersionTable"></div></article></section>
        ...
        <section class="mock-timeline-layout">
          <article class="modern-card">
            <div class="section-title">
              ...
              <span class="badge-soft" id="mockEventCount">0 条事件</span>
            </div>
