...
async function loadEngineeringSession(){
  ...
  try{
    ...
    if(badge) badge.textContent = session.status === 'Active' ? '已认证' : session.status;
...
async function loadEngineeringDiagnostics(){
  ...
  engineeringTable('engineeringCommandLog', ['时间','命令','状态','运行','版本'], (commands.items || []).map(x => `<div class="table-row"><span>${escapeHtml(formatDateTime(x.createdAtUtc))}</span><span>${escapeHtml(x.commandType)}</span><span>${escapeHtml(x.status)}</span><span>${escapeHtml(x.machineRunId || '--')}</span><span>${escapeHtml(x.liquidClassVersionId || '--')}</span></div>`), '暂无命令记录');
  engineeringTable('engineeringErrors', ['时间','来源','代码','状态','运行'], (errors.items || []).map(x => `<div class="table-row"><span>${escapeHtml(formatDateTime(x.createdAtUtc))}</span><span>${escapeHtml(x.sourceType)}</span><span title="${escapeHtml(x.message)}">${escapeHtml(x.code)}</span><span>${escapeHtml(x.status)}</span><span>${escapeHtml(x.machineRunId || '--')}</span></div>`), '暂无错误码');
  engineeringTable('engineeringCommunications', ['时间','模块/动作','设备状态','持久化','报文'], (communications.items || []).map(x => `<div class="table-row"><span>${escapeHtml(formatDateTime(x.createdAtUtc))}</span><span>${escapeHtml(x.moduleCode)} / ${escapeHtml(x.action)}<small>${escapeHtml(x.adapterName)}</small></span><span>${escapeHtml(x.status)}<small>${escapeHtml(x.errorCode || '')}</small></span><span>${escapeHtml(x.persistenceStatus)}<small>${escapeHtml(x.persistenceFailureReason || '')}</small></span><span><button class="btn btn-soft" data-communication-id="${escapeHtml(x.id)}">查看技术报文</button></span></div>`), '暂无 Mock 通讯记录');
...
async function loadEngineerPage(){
  try{
    ...
    setText('engineerRunState', currentRun ? `${currentRun.runCode || currentRun.id} / ${currentRun.status}` : '无活动任务');
